...
export class AvisService {
  ...
  LOADNBParticipant(): Observable<number> {
    return this.http.get<number>(this.loadNBParticipantURL);
  }
  ...
  AddAvis(Avis: Avis): Observable<Avis> {
    return this.http.post<Avis>(this.AddAvisURL, Avis, httpOptions);
  }

  loadAvisByIdTheme(id: number): Observable<Avis[]> {
    return this.http.get<Avis[]>("http://localhost:8080/FEEDBACK/api/loadAvisByIdTheme/" + `${id}`)
  }
  loadMoyenneRate(id: number): Observable<number[]> {
    return this.http.get<number[]>("http://localhost:8080/FEEDBACK/api/loadMoyenneRate/" + `${id}`)
  }

  addTheme(theme: ThemeAvis): Observable<ThemeAvis> {
    return this.http.post<ThemeAvis>(this.AddThemes1URL, theme, httpOptions);
  }
